...
bool enable_color_euclidean_clusters_and_get_3D_bounding_boxes = true;
...
void color_euclidean_clusters_and_get_3D_bounding_boxes(const pcl::PCLPointCloud2ConstPtr& segmented_objects_cloud,
  ...
  if(!enable_color_euclidean_clusters_and_get_3D_bounding_boxes){
  ...
  for(int i=0; i<cluster_indices.size(); ++i){
    ...
    *indexes = cluster_indices[i];
    std::cout << "Object " << i<< "has " <<indexes->indices.size() << " points" << std::endl;
...
void cloud_cb (const pcl::PCLPointCloud2ConstPtr& input_cloud)
{
  ...
  region(afterSeg_objects_outlier_removed_cloud, region_cloud);
...
void dyanamic_reconfigure_callback(object_detection::pcl_parametersConfig &config, uint32_t level) {
  ...
  enable_color_euclidean_clusters_and_get_3D_bounding_boxes = config.enable_color_euclidean_clusters_and_get_3D_bounding_boxes;
