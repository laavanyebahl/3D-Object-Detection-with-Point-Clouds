...
We perform object segmentation on 3D point cloud data using `python-pcl` to leverage the power of the [Point Cloud Library](http://pointclouds.org/).  We perform filtering and RANSAC plane segmentation. We then write a ROS node to perform those functions as well as Euclidean Clustering for object segmentation! 
...
`pcl_helper.py` contains useful functions for working with point cloud data with ROS and PCL. Here's a brief description of the contents:
